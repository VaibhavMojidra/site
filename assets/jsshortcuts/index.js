...
const combinations = e => {
    if (navigator.platform.match("Mac") ? e.metaKey : e.ctrlKey) {
        ...
        switch (e.key) {
            case '1': {
                e.preventDefault()
                window.location.href = 'https://www.instagram.com/vaibhavmojidra2311/'
            }
                break
            case '2': {
                e.preventDefault()
                window.location.href = 'https://www.facebook.com/vaibhavmojidra2311'
            }
                break
            case '3': {
                e.preventDefault()
                window.location.href = 'https://twitter.com/vaibhavmojidra'
            }
                break
            case '4': {
                e.preventDefault()
                window.location.href = 'https://www.linkedin.com/in/vaibhav-mojidra-260733171'
            }
                break
            case '5': {
                e.preventDefault()
                window.location.href = 'https://www.snapchat.com/add/vaibhavmojidra'
            }
                break
            case '6': {
                e.preventDefault()
                window.location.href = 'https://play.google.com/store/apps/dev?id=5230170147318032032'
            }
                break
            case '7': {
                e.preventDefault()
                window.location.href = 'https://www.youtube.com/channel/UCqDyWIOj8Rl1Xg7NwYy3A-g'
            }
                break
            case '8': {
                e.preventDefault()
                window.location.href = 'https://github.com/VaibhavMojidra'
            }
                break
            case '9': {
                e.preventDefault()
            ...
            case '0': {
                e.preventDefault()
                window.location.href = 'https://stackoverflow.com/users/13474737/vaibhav-mojidra'
            }
                break
        }
    }
    ...
    if (status === false) {
        setTimeout(() => {
            ...
            setTimeout(() => {
                closeNotificaion()
                setTimeout(() => {
                    showNotificaion('Do try <b>Smart Search</b> feature by clicking on <b>Vaibhav Mojidra</b> on the top-left of the page.')
                    setTimeout(() => {
                        closeNotificaion()
                        setTimeout(() => {
